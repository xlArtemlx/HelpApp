...
const SwitchMonth = ({prevActiveDate, nextActiveDate, setWeeksTC, weeks}) => {
    ...
    return (
        <View style={styles.switchMonth}>
            <TouchableOpacity onPress={() => handlePress('prev')}>
                {<AntDesign name="arrowleft" size={24} color='#1768AF'/>}
            </TouchableOpacity>

            <View style={{alignItems: 'center'}}>
                <Text style={styles.month}>{prevActiveDate.format('MMMM')}</Text>
                <Text style={styles.weekdays}>{_startDay} - {_endDay}</Text>
            </View>

...
const DateNumber = ({date, setDate, theme}) => {
    ...
    return (
        <View style={{...styles.date, backgroundColor: theme && theme.navigator}}>
            <TouchableOpacity onPress={() => _changeDate('prev')}>
                <Image style={styles.arrowIcon} source={require('../../images/icons/ic_arrow_left.png')}/>
            </TouchableOpacity>

            <View style={styles.textDate}>
                <Text style={{...styles.textDay, color: theme && theme.text}}>{date.locale(activeLanguage).format('DD MMM')}</Text>
                <Text style={{color: theme && theme.text}}>{date.locale(activeLanguage).format('dddd')}</Text>
            </View>
